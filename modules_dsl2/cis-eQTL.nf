...
process cisQTL_nominal {
 ...
 output_s_mod = output_s - ~/Geno/
 output_t_mod = output_t - ~/Geno/
 output_g_mod = output_g - ~/Geno/
...
QTLtools cis --vcf $genotype_s --bed $phenotype_s --cov $covariate_s --nominal 0.01 --normal --out ${output_s}

sed '1iPhenotypeID_Gene Chrmosome start end strand TotalTestVariants Distance VariantId VarinatChrId start end NominalP-val RegressionSlope Binaryflag' ${output_s} >  FBN_liver_${output_s_mod}_cis.nominal.txt
...
sed '1iPhenotypeID_Gene Chrmosome Start End Strand TotalTestVariants Distance VariantId VarinatChrId VariantStart VariantEnd DF Dummy 1Parbeta 2Parbeta unknown NominalP-val RegressionSlope EmpiricalP-val adjP-val' ${output_s} >  FBN_liver_${output_s_mod}_cis.permu.txt
