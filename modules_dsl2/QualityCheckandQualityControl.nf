...
process fastqc {
    ...
    script:
    name = signleEndread_files[0].toString() - ~/(R1_)?(001)?(\.fastq)?(\.gz)?$/
    prefix = name.substring(0,5)
    """
    mkdir fastqc_${prefix}_logs
    fastqc -o fastqc_${prefix}_logs -f fastq -q ${signleEndread_files}
    """
}
                                    /*
...
process trimmomatic{
  ...
  container 'sglim2/trimmomatic-0.36'
  publishDir "${params.outdir}/Trimmomatic", mode:'copy'

  input:
      tuple val(prefix), file(readPair_files) 
      file trimmjar  
      file adapter 
  output:
...
process trimmomatic_singleEnd{
  ...
  output:
     ...
     //tuple val(prefix), file ("${prefix}_R1_unpaired.fastq.gz")
     tuple val(prefix), file ("${prefix}_trimomatic.log")

  script:
   //name = signleEndread_files[0].toString() - ~/(R1_)?(001)?(\.fastq)?(\.gz)?$/
   
   name = signleEndread_files[0].toString()
   prefix = name.split('_')[0]

  """
   java -jar ${trimmjar} SE -phred33 ${signleEndread_files} ${prefix}_SingleEnd.clean.fastq.gz -threads 10 \
   ILLUMINACLIP:${adapter}:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36 2> ${prefix}_trimomatic.log
  """
}
