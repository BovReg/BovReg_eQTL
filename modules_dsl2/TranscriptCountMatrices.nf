...
process transcriptGTFfiles {
  tag "TranscriptGTFfiles on $gtf_input"
  input:
   file (gtf_input)  

  output:

   file ("${outtranscriptTbl}_transcriptCounts.txt") 

  script:

  outtranscriptTbl = gtf_input[0].toString() - ~/(\_stringtie.gtf)?$/
  """
  awk -v OFS='\t' 'NR==1 {gsub(/_stringtie.gtf/,"",\$11) ;print \$1="Chr",\$2="start",\$3="end",\$4="pid",\$5="gid",\$6="strand",\$7=\$11 }; \
  NR>1{if(\$3=="transcript" && \$19=="TPM") print \$1,\$4,\$5,\$12,\$10,\$7,\$20 ; \
  else if(\$3=="transcript" && \$17=="TPM") print \$1,\$4,\$5,\$12,\$10,\$7,\$18}' ${gtf_input} \
   | sed 's/"//g;s/;//g' > "${outtranscriptTbl}_transcriptCounts.txt"
  """
}
...
process mergeNormalizedTranscriptCountMatrices {
  ...
  awk 'NR==1;NR>1   {z=0; for (i=7; i<=NF; ++i) if (\$i < 1 && ++z> NF-9 ) next; print }'  Transcript_count_Matrices.tsv \
