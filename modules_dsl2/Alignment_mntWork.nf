 ...
 process starAlign_GeneTranscript{
    ...
    input:
    tuple val(sample_id), file(readPair_files)
    file gtf 
    file index
    ...
    script:
    prefix = readPair_files[0].toString() - ~/(\.clean)?(\.fastq)?(\.gz)?$/
    ...
    """
     zcat $readPair_files > ${prefix}.fastq.gz

     STAR --genomeDir ${index} \
             --sjdbGTFfile ${gtf} \
             --readFilesIn ${prefix}.fastq.gz \
             --runThreadN 10 \
 ...
 process starAlign_Splicing {
    ...
    script:
    prefix1 = read1[0].toString() - ~/(\.clean)?(\.fastq)?(\.gz)?$/

    prefix2 = read2[0].toString() - ~/(\.clean)?(\.fastq)?(\.gz)?$/
    ...
    zcat $read1 > ${prefix1}.clean.fastq

    zcat $read2 > ${prefix2}.clean.fastq 
      
     STAR --genomeDir ${index} \
             --sjdbGTFfile ${gtf} \
             --readFilesIn  ${prefix1}.clean.fastq ${prefix2}.clean.fastq \
             --twopassMode Basic \
 ...
 process starAlign_unpaired_R1 {
    ...
    script:
     sample_id = unpaired_read1[0].toString() - ~/(_R1)?(\_unpaired)?(\.fastq)?(\.gz)?$/
...
process starAlign_unpaired_R2 {
    ...
    script:
    sample_id = unpaired_read2[0].toString() - ~/(_R2)?(\_unpaired)?(\.fastq)?(\.gz)?$/
